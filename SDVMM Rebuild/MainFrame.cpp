...
void MainFrame::LoadModsFromDir(string folder_name) 
{
	...
	for (auto& dir_iter : fs::recursive_directory_iterator(temp_dir))
	{
		...
		DPRINT(temp_path.string());
		if (fs::is_directory(temp_path))
		{
			DPRINTIF(report_identify_directories, " - Is Directory\n");
			...
			if (fs::exists(temp_path))
			{
				...
				try
				{
					...
					{
						DPRINTIF(report_parsed_mod_data, 
						...
						*/
						...
						DPRINTIF(report_mod_object_data, aMod.infoString());
					}
				}
				catch (...)
				{
